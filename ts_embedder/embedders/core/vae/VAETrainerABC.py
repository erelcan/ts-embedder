...
from ts_embedder.embedders.core.training.TrainerABC import TrainerABC
...
class VAETrainerABC(TrainerABC):
    ...
    def train(self):
        ...
        if self._model_info["has_implicit_loss"]:
            model.compile(optimizer=self._model_info["optimizer"], metrics=self._model_info["metrics"])
        else:
            model.compile(optimizer=self._model_info["optimizer"], loss=get_loss_function(self._model_info["loss_info"]), metrics=self._model_info["metrics"])
